...
namespace PredictorPatchFramework
{
    public class PredictionItem
    {
        ...
        {
            Draw(spriteBatch, Texture, SourceRect, pos, scale, Stack, Quality, layer);
        }
        ...
        public static void Draw(SpriteBatch spriteBatch, ISalable item, Vector2 pos, Vector2 scale, int stack = 1, int quality = 0, float layer = 0f)
        {
            var data = ItemRegistry.GetData(item.QualifiedItemId) ?? throw new ArgumentException($"{item.Name} has no data.");
            Draw(spriteBatch, data.GetTexture(), data.GetSourceRect(), pos, scale, stack, quality, layer);
        }

        public static void Draw(SpriteBatch spriteBatch, Texture2D texture, Rectangle sourceRect, Vector2 pos, Vector2 scale, int stack = 1, int quality = 0, float layer = 0f)
        {
            var ratio = TextureSize / (float)Math.Max(sourceRect.Width, sourceRect.Height);
            var scale1 = Math.Max(scale.X, scale.Y) * ratio;
            spriteBatch.Draw(texture, pos, sourceRect, FrameworkUtils.API.ItemColor, 0f, Vector2.One * 0.5f, scale1, SpriteEffects.None, layer);
            if (stack > 1)
            {
                Utility.drawTinyDigits(stack, spriteBatch, pos, scale1, 0f, FrameworkUtils.API.TextColor);
            }
            if (quality > 0)
            {
                Rectangle value = quality < 4 ? new Rectangle(338 + (quality - 1) * 8, 400, 8, 8) : new Rectangle(346, 392, 8, 8);
                spriteBatch.Draw(Game1.mouseCursors, pos + new Vector2(8, 16) * scale1, value, Color.White, 0f, new Vector2(4f, 4f) * scale, scale1, SpriteEffects.None, layer);
            }
        }
