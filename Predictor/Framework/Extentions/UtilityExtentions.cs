...
namespace Predictor.Framework.Extentions
{
    internal static class UtilityExtentions
    {
        public static bool Predict_trySpawnRareObject(PredictionContext _ctx, Farmer? who, Vector2 position, GameLocation location, double chanceModifier = 1.0, double dailyLuckWeight = 1.0, int groundLevel = -1, Random? random = null)
        {
            ...
            {
                Game1Extentions.Predict_createItemDebris(_ctx, PredictionItem.Create("(O)GoldenAnimalCracker"), position, -1, location, groundLevel);
            }

            if (Game1.stats.DaysPlayed > 2 && random.NextDouble() < 0.002 * chanceModifier)
            {
                Game1Extentions.Predict_createItemDebris(_ctx, new PredictionItem(Utility.getRandomCosmeticItem(_ctx.Random)), position, -1, location, groundLevel);
            }

            if (Game1.stats.DaysPlayed > 2 && random.NextDouble() < 0.0006 * chanceModifier)
            {
                Game1Extentions.Predict_createItemDebris(_ctx, PredictionItem.Create("(O)SkillBook_" + _ctx.Random.Next(5)), position, -1, location, groundLevel);
            }
