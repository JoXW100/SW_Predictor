...
{
    internal static class TreeExtentions
    {
                ...
                if (tree.growthStage.Value >= 5)
                {
                    ...
                    if (tree.hasSeed.Value && (Game1.IsMultiplayer || Game1.player.ForagingLevel >= 1))
                    {
                        ...
                        if (data != null && data.SeedDropItems?.Count > 0)
                        {
                            foreach (WildTreeSeedDropItemData seedDropItem in data.SeedDropItems)
                            {
                                ...
                                if (item != null)
                                {
                                    ...
                                    Game1Extentions.Predict_createItemDebris(_ctx, new PredictionItem(item), new Vector2(tileLocation.X * 64f, (tileLocation.Y - 3f) * 64f), -1, location, Game1.player.StandingPixel.Y);
                        ...
                        if (flag && data != null)
                        {
                            ...
                            Game1Extentions.Predict_createItemDebris(_ctx, new PredictionItem(item2), new Vector2(tileLocation.X * 64f, (tileLocation.Y - 3f) * 64f), -1, location, Game1.player.StandingPixel.Y);
                        }

                        if (Utility.tryRollMysteryBox(0.03))
                        {
                            Game1Extentions.Predict_createItemDebris(_ctx, PredictionItem.Create((Game1.player.stats.Get(StatKeys.Mastery(2)) != 0) ? "(O)GoldenMysteryBox" : "(O)MysteryBox"), new Vector2(tileLocation.X, tileLocation.Y - 3f) * 64f, -1, location, Game1.player.StandingPixel.Y - 32);
                        }

                        UtilityExtentions.Predict_trySpawnRareObject(_ctx, Game1.player, new Vector2(tileLocation.X, tileLocation.Y - 3f) * 64f, tree.Location, 2.0, 1.0, Game1.player.StandingPixel.Y - 32);
                        if (_ctx.Random.NextBool() && Game1.player.team.SpecialOrderRuleActive("DROP_QI_BEANS"))
                        {
                            Game1Extentions.Predict_createObjectDebris(_ctx, "(O)890", (int)tileLocation.X, (int)tileLocation.Y - 3, ((int)tileLocation.Y + 1) * 64, 0, 1f, location);
                        }
...

                    {
                        foreach (WildTreeItemData shakeItem in data.ShakeItems)
                        {
                            ...
                            if (item3 != null)
                            {
                                Game1Extentions.Predict_createItemDebris(_ctx, new PredictionItem(item3), tileLocation * 64f, -2, tree.Location);
                            }
                        }
