...
{
    internal static class BushExtentions
    {
        ...
        public static void Predict_shake(this Bush bush, PredictionContext _ctx, Vector2 tileLocation)
        {
            if (bush.IsHarvestable())
            {
                ...
                {
                    case 4:
                        Game1Extentions.Predict_createItemDebris(_ctx, PredictionItem.Create(shakeOff), new Vector2(bush.getBoundingBox().Center.X, bush.getBoundingBox().Bottom - 2), 0, bush. Location, bush.getBoundingBox().Bottom);
                        break;
                    case 3:
                        Game1Extentions.Predict_createObjectDebris(_ctx, shakeOff, (int)tileLocation.X, (int)tileLocation.Y);
                        break;
                    default:
                        {
                            ...
                            for (int i = 0; i < num; i++)
                            {
                                ...
                                Game1Extentions.Predict_createItemDebris(_ctx, item, Utility.PointToVector2(bush.getBoundingBox().Center), Game1.random.Next(1, 4));
                            }
                            break;
                        }
                }
