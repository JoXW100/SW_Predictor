...
namespace PredictorCropPatch.Extentions
{
    public static class HarvestExtentions
    {
        ...
        public static void Predict_harvestSpawned(this Object obj, PredictionContext _ctx, Farmer who, Vector2 tileLocation)
        {
            ...
            Random random = Utility.CreateDaySaveRandom(tileLocation.X, tileLocation.Y * 777f);
            var item = new PredictionItem(obj);
            if (who.professions.Contains(16))
            {
                item.Quality = 4;
            }
            else if (random.NextDouble() < (double)(who.ForagingLevel / 30f))
            {
                item.Quality = 2;
            }
            ...
            CreateItemExtentions.Predict_createItemDebris(_ctx, item, vector, -1);
        }
    }
}
